...
void main() {
  ...
  group('Bytes Uint64 Tests', () {

    test('DicomReadBuffer BytesDicomLE Uint64 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint64List(min, max);
        final rBuf0 = getReadBufferLE(vList0);
        final out = Uint64List(vList0.length);

        for (var j = 0; j < vList0.length; j++) {
          final v = rBuf0.readUint64();
        ...
        final vList1 = rBuf1.readUint64List(vList0.length);
        expect(vList1, equals(vList0));
      }
    });

    test('DicomReadBuffer BytesDicomBE Uint64 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint64List(min, max);
