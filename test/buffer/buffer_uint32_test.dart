...
void main() {
  ...
  group('Bytes Uint32 Tests', () {

    test('DicomReadBuffer BytesDicomLE Uint32 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint32List(min, max);
        final rBuf0 = getReadBufferLE(vList0);
        final out = Uint32List(vList0.length);

        for (var j = 0; j < vList0.length; j++) {
          final v = rBuf0.readUint32();
        ...
        final vList1 = rBuf1.readUint32List(vList0.length);
        expect(vList1, equals(vList0));
      }
    });

    test('DicomReadBuffer BytesDicomBE Uint32 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint32List(min, max);
