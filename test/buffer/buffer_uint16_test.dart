...
void main() {
  ...
  group('Bytes Uint16 Tests', () {
    ...
    test('DicomReadBuffer BytesDicomLE Uint16 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint16List(min, max);
        final rBuf0 = getReadBufferLE(vList0);
        final out = Uint16List(vList0.length);

        for (var j = 0; j < vList0.length; j++) {
          final v = rBuf0.readUint16();
        ...
        final vList1 = rBuf1.readUint16List(vList0.length);
        expect(vList1, equals(vList0));
      }
    });

    test('DicomReadBuffer BytesDicomBE Uint16 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.uint16List(min, max);
