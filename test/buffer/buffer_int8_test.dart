...
void main() {
  ...
  group('Bytes Int8 Tests', () {
    ...
    test('DicomReadBuffer BytesDicomLE Int8 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.int8List(min, max);
        final rBuf0 = getReadBufferLE(vList0);
        final out = Int8List(vList0.length);

        for (var j = 0; j < vList0.length; j++) {
          final v = rBuf0.readInt8();
        ...
        final vList1 = rBuf1.readInt8List(vList0.length);
        expect(vList1, equals(vList0));
      }
    });

    test('DicomReadBuffer BytesDicomBE Int8 tests', () {
      for (var i = 0; i < repetitions; i++) {
        final vList0 = rng.int8List(min, max);
