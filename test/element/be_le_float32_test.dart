...
void main() {
  ...
  group('BytesLittleEndian Float32 FL', () {
    test('LE Float32 FL tests', () {
      ...
      for (var i = 0; i < repetitions; i++) {
        ...
        final vf = Bytes.typedDataView(vList0);
        expect(vList0.buffer == vf.buffer, isTrue);
        expect(isAligned32(vf.buf.offsetInBytes), isTrue);
        final e = BytesLELongEvr.element(0x00080008, kFLCode, vf);
        print('e: $e');

        final bytes0 = e.asBytes();
        expect(bytes0.endian == Endian.little, isTrue);
        expect(bytes0.length == e.length, isTrue);
        expect(bytes0.buffer == e.buffer, isTrue);

        final vList1 = e.buffer.asFloat32List();
